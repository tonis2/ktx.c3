...
the error stays in codec-expected bounds. The whole round-trip runs twice:
once on the main thread and once on a worker thread — FFI hosts (e.g. the
Blender addon) call in from thread pools, where C3 has no implicit temp
allocator and an unguarded entry point would abort the host process.
Run by CI after build-shared.sh.
"""
import ctypes
import sys
import threading
...
def roundtrip(tag):
    ...
    for fmt, tol in (("rgba8-srgb", 0.0), ("bc7-srgb", 4.0), ("uastc", 4.0), ("etc1s", 16.0)):
        ...
        if lib.ktx_encode(src, W, H, fmt.encode(), 1, 0, 90, 2, 0,
            ...
            print(f"FAIL {fmt} [{tag}]: encode: {lib.ktx_last_error().decode()}")
            failures += 1
            continue
        ...
        if lib.ktx_decode(blob, len(blob), 0, 0, 0,
            ...
            print(f"FAIL {fmt} [{tag}]: decode: {lib.ktx_last_error().decode()}")
            failures += 1
            continue
        ...
        if (w.value, h.value) != (W, H):
            print(f"FAIL {fmt} [{tag}]: size {w.value}x{h.value}")
            failures += 1
            continue
        mad = sum(abs(a - b) for a, b in zip(rgba, src)) / len(src)
        ok = mad <= tol
        print(f"{'ok  ' if ok else 'FAIL'} {fmt:10s} [{tag}] {len(blob):6d} bytes  mad={mad:.2f} (tol {tol})")
        failures += 0 if ok else 1
    return failures


failures = roundtrip("main thread")

worker_failures = []
t = threading.Thread(target=lambda: worker_failures.append(roundtrip("worker thread")))
t.start()
t.join()
# An empty list means the worker died mid-run (a C3 panic aborts the process
# outright, but guard the soft-failure path too).
failures += worker_failures[0] if worker_failures else 1
